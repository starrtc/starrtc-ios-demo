...
@implementation XHEchoCancellation
...
{
    static XHEchoCancellation *cancel = nil;
...
- (instancetype)init
{
    if (self = [super init])
    {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            _echoCancellationStatus = XHEchoCancellationStatus_close;
...
- (void)stop
{
    //AudioComponentInstanceDispose(myStruct.remoteIOUnit);
    self.bl_input = nil;
    self.bl_output = nil;
...
-(void)openOrCloseEchoCancellation:(UInt32)newEchoCancellationStatus
{
    ...
    _echoCancellationStatus = newEchoCancellationStatus == 0 ? XHEchoCancellationStatus_open : XHEchoCancellationStatus_close;
}
...
-(void)startGraph:(AUGraph)graph
{
    ...
    CheckError(AUGraphStart(graph),
               "AUGraphStart failed");
    _echoCancellationStatus = XHEchoCancellationStatus_open;
}
...
OSStatus InputCallback_xb(void *inRefCon,
    ...
    XHEchoCancellation *echoCancellation = (__bridge XHEchoCancellation*)inRefCon;
