...
@property (nonatomic, strong) XHWhitePanel *whitePanel;
@end
...
- (void)viewDidLoad {
    ...
    XHWhitePanel *panelManager = [[XHWhitePanel alloc] init];
...
//    [scrollView addSubview:chatView];
    ...
    UIButton *whitePanelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [whitePanelBtn setImage:[UIImage imageNamed:@"btn_panel_n"] forState:UIControlStateNormal];
    [whitePanelBtn setImage:[UIImage imageNamed:@"btn_panel_s"] forState:UIControlStateSelected];
    [whitePanelBtn addTarget:self action:@selector(whitePanelBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    whitePanelBtn.hidden = YES;
    [self.view addSubview:whitePanelBtn];
    [whitePanelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [cleanBtn setImage:[UIImage imageNamed:@"btn_xp"] forState:UIControlStateNormal];
    [cleanBtn addTarget:self action:@selector(cleanBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    cleanBtn.hidden = YES;
    [self.view addSubview:cleanBtn];
    self.cleanBtn = cleanBtn;
    [cleanBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(whitePanelBtn.mas_left).offset(-5);
        make.bottom.equalTo(whitePanelBtn);
        make.size.mas_equalTo(CGSizeMake(44, 44));
    }];
    if (_vcType != IFLiveVCTypeLook) {
        whitePanelBtn.hidden = NO;
    }
}
...
- (void)whitePanelBtnClicked:(UIButton *)button {
    button.selected = !button.selected;
    
    self.cleanBtn.hidden = !button.isSelected;
...
- (void)cleanBtnClicked:(UIButton *)button {
