...
@interface SenderVoipVideoVC ()<StarManagerModelVoipdelegate>
...
- (void)viewDidLoad {
    ...
    [m_starManagerModel initVoip:YES userID:[IMUserInfo shareInstance].userID targetID:_toUserID callback:^(BOOL reqSuccess, NSString *statusCode, NSString *data) {
        if(reqSuccess)
        {
            NSLog(@"连接成功");
        }
...
{
    if (_contectType != -1) {
        [m_starManagerModel initVoip:YES userID:[IMUserInfo shareInstance].userID targetID:toUserId callback:^(BOOL reqSuccess, NSString *statusCode, NSString *data) {
            if(reqSuccess)
            {
                NSLog(@"连接成功");
            }
...
//接收连接
- (void)receiverVoipToUserId:(NSString*) toUserId
{
    if (_contectType != -1) {
        [m_starManagerModel initVoip:NO userID:[IMUserInfo shareInstance].userID targetID:toUserId callback:^(BOOL reqSuccess, NSString *statusCode, NSString *data) {
            if(reqSuccess)
            {
                NSLog(@"连接成功");
            }
